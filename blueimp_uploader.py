        ...
        for name, fieldStorage in self.request.POST.items():
            ...
            if self.validate(result):
                ...
                result['deleteUrl'] = self.request.host_url +\
                    '/' + globalDict['relUriUpload'] + '?key=' + urllib.quote(blob_key, '')
...
class BlueimpUploadPage(webapp2.RequestHandler):
  def get(self):
    if(1):
      template = JINJA_ENVIRONMENT.get_template('templates/blueimp_uploader_index.html');
      template_values = {
          'url'     : self.request.host_url,
          }
      self.response.write('<h1>template</h1>');
      self.response.write(template.render(template_values));

app = webapp2.WSGIApplication(
    [
        ('/' + globalDict['upload_page'],                         BlueimpUploadPage),
        ('/' + globalDict['relUriUpload'],                        UploadHandler),
        ('/' + globalDict['relUriDownload'] + '/([^/]+)/([^/]+)', DownloadHandler)
    ],
    debug=True
)
